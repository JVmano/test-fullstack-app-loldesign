...
export function calculatePlan (data: calculatePlanDTO) {
  ...
  const baseValueWithPlan = data.pricePerMinute * (Number(data.callTime) - Number(data.plan))

  // if the total minutes exceeded the plan's maximum discount apply 10% penality
  // otherwise only left the remainder of value
  const withPlanValue = baseValueWithPlan > 0 ? baseValueWithPlan + baseValueWithPlan * 0.1 : 0
  ...
  return { noPlanValue: formattedNoPlanCallValue, planValue: formattedWithPlanValue }
}
